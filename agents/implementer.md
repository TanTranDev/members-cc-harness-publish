...
này nằm ở tích luỹ turn vì subagent tự đi tìm (bộ luật §0 "Bàn giao cho subagent"). Thiếu thông tin
...
4. **Gate cuối — đúng 1 LẦN trên diff cuối** (bộ luật §0 "Verify & Review"): main sẽ spawn `verifier` ⇒ bạn KHÔNG tự chạy full gate; không có verifier ⇒ tự chạy `npm run typecheck && npm run lint && npm test && npm run structure && npm run spec` và ghi ledger `docs/wip/<task>/verify.md`. KHÔNG báo "xong" khi chưa có ledger/bằng chứng.
...
- TextInput: không set `lineHeight`, không `includeFontPadding: false`, `minHeight ≥ 48` (chống crop descender với font có descender dài — nguyên lý CLAUDE.md §6, font cụ thể PROJECT.md).
